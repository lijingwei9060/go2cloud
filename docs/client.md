...
client.exe test_vss              # 测试 VSS 快照功能
...
**成功输出**：
```
Using VSS volume: \\?\GLOBALROOT\Device\HarddiskVolumeShadowCopy123
```
退出码 0。
...
**内部步骤**：
1. `vss_init()` — 加载 vssapi.dll，初始化 COM
2. `vss_create_snapshots(ctx, {"C:", NULL})` — 创建 `C:` 卷的拷贝快照（`VSS_BT_COPY`，不截断日志）
3. 打印快照设备路径
4. `vss_backup_complete()` + `vss_cleanup()`

### 3.4 sentbytes — 跨进程进度查询
...
### 3.5 begin_session / end_session — 会话控制
...
├─ client.exe test_vss          → 验证 VSS 可用性
