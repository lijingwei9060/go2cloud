...
**`end_session`**：迁移完成或取消后调用。执行 `DELETE FROM T_BLOCK` 清空块跟踪表，T_VERSION 表保留不删除。**注意**：不释放 VSS 快照 — 快照由数据面进程退出时随进程销毁。
...
- `flags`：
  - `--pending`：仅显示 ack=0 块
  - `--acked`：仅显示 ack=1 块
  - `--version=N`：仅显示指定版本号的块
  - `--history`：显示 T_VERSION 轮次历史（仅整体摘要模式有效）
...
-------- ---------------- ---------- ------------------ --- ---
0        0                1048576    0x3a7b2c1d8e4f5a6b 1   0
0        1048576          1048576    0x8f1e2d3c4b5a6c7d 1   1
0        2097152          1048576    0x1a2b3c4d5e6f7a8b 1   2
...
```
...
Database: tracker.db
  ...
  ----- -------------------- -------------------- -------- -------- ----------
  0     2026-07-01 10:00:00  2026-07-01 10:30:00  153600   153600   1800s
  1     2026-07-02 09:00:00  2026-07-02 09:05:00  153600   42       300s
  2     2026-07-02 09:15:00  2026-07-02 09:15:15  153600   3        15s
```

版本 0 为全量同步，版本 1+ 为增量轮次。`--history` 仅在整体摘要模式（无 devno 参数）下有效。
...
4. 打开跟踪数据库，自动获取下一个版本号（`COALESCE(MAX(version),0)+1`），在 T_VERSION 中记录本轮开始
5. 逐分区逐块：读 live disk → hash → 对比 DB → hash 相同则标记为本轮已验证（`sqlite_block_mark_verified`），hash 不同则发送新数据并写入新版本号
6. 排空队列，发送 `ctlEndIncremental`，等待服务端确认
7. 记录本轮结束（扫描数、变化数），打印统计：版本号、扫描数、跳过数、变化数、发送字节数
...
- 全量同步完成后，进入独立的增量收敛循环
- 每轮开始：自动获取下一个版本号（自增），在 T_VERSION 表中记录开始时间
- 从 live disk 重读所有块，对比哈希
- hash 相同的块标记为本轮已验证（`version=N, ack=1`），无需 reset_acked
- 仅重发变化的块
- 收敛判定后退出（见 5.3.2）
...
| 条件 | 判定标准 | 说明 |
|------|---------|------|
| **完美收敛** | `unacked == 0` (仅限 version < current_version) | 所有块在当前轮前已确认 |
...
CREATE TABLE IF NOT EXISTS T_BLOCK(
    ...
    version   INTEGER DEFAULT 0,  -- 同步轮次版本号 (0=全量, 1+=增量)
...
CREATE TABLE IF NOT EXISTS T_VERSION(
    ...
    start_time    INTEGER,              -- 本轮开始时间 (毫秒时间戳)
    end_time      INTEGER,              -- 本轮结束时间 (毫秒时间戳)
    scanned_count INTEGER DEFAULT 0,    -- 扫描块数
    changed_count INTEGER DEFAULT 0     -- 变化块数 (重发数)
);
...
- 首次同步（表为空）→ version = 1
- 第二次 incsync → version = 2
- 不同服务端各自计数（通过 remote_id 区分）

`cmd_incsync` 和 `do_migrate` 的增量循环均在开始时自动获取下一个版本号，无需手动指定。
...
| 插入/更新 (带版本) | `sqlite_block_upsert_v(..., version)` | `INSERT OR REPLACE` — 通过唯一索引实现 upsert，写入指定版本号 |
| 标记已验证 | `sqlite_block_mark_verified(devno, offset, version)` | 增量同步中 hash 未变化的块：设 `version=N, ack=1` |
| 标记已确认 | `sqlite_block_mark_acked(devno, offset)` | 服务端 ACK 后调用，设 `ack=1`（全量同步用） |
...
| 统计未确认 (版本) | `sqlite_count_unacked_v(db, remote_id, before_version)` | 收敛判定：统计 `ack=0 AND version < before_version` 块数 |
| 查询已确认字节 | `sqlite_total_acked_bytes(db)` | `sentbytes` 子命令：跨进程进度查询 |
| 清空表 | `sqlite_clear_all_blocks(db)` | `end_session` 子命令：清理块数据（T_VERSION 保留） |
| 版本号获取 | `sqlite_get_next_version(db, remote_id)` | 自动递增：`COALESCE(MAX(version),0)+1` |
| 版本开始 | `sqlite_version_start(db, ver, remote_id, ts)` | 插入 T_VERSION 行，记录开始时间 |
| 版本结束 | `sqlite_version_end(db, ver, remote_id, ts, scanned, changed)` | 更新 T_VERSION 行，记录结束时间和统计 |
| 版本历史 | `sqlite_get_version_history(db, remote_id, infos, max)` | 查询 T_VERSION 表所有轮次信息 |
| 按版本统计 | `sqlite_count_blocks_by_version(db, ver, remote_id)` | 统计指定版本号的块数 |
| 块信息查询 | `sqlite_get_block_info()` / `sqlite_list_blocks()` | `blockinfo` 子命令：逐块检查状态（含 version 字段） |
| 块数统计 | `sqlite_count_blocks(devno, ack)` | `blockinfo` 子命令：按磁盘/确认状态统计 |

### 6.4 ACK 语义与生命周期

```
INSERT (ack=0, version=N, last_sent=ts)    ← 块首次发送 / 变化块重发
      │
      ▼
SELECT WHERE ack=0 AND version < current_version   ← 增量轮次查询未确认块
      │
      ├─ hash 未变 → UPDATE version=current_version, ack=1   ← 标记已验证
      │
      ├─ hash 变化 → 重新发送 (新 version)     ← ack 保持 0，等待 ACK
      │              │
      │              ▼
      │         服务端 ACK → UPDATE ack=1      ← 服务端确认
      │
      ▼
下一轮: 自动获取新版本号，无需 reset_acked
```

**关键设计变更**（与旧版对比）：
- **不再使用 `sqlite_reset_acked`**：旧版每轮开始时将所有 `ack=1` 重置为 `ack=0`，丢失历史。新版通过版本号区分轮次，`ack` 仅跟踪单轮内的服务端确认状态。
- **T_BLOCK 永久保留**：迁移完成后不再清空 T_BLOCK，数据在后续 `incsync` 中持续复用。
- **T_VERSION 独立存在**：`end_session` 仅清空 T_BLOCK，T_VERSION 历史记录永久保留。
...
| 未确认块来源 | `SELECT ... WHERE ACK=0` | 同 + 每轮新版本号，查询 `ack=0 AND version < current_version` |
| 哈希对比后 | 不变 → 跳过（ACK 保持 0）; 变 → 重发（ACK 保持 0） | 不变 → `mark_verified(version=N)`; 变 → 重发（新 version） |
| 收敛检测 | `timer_cb` 轮询 `SUM(SIZE) WHERE ACK=0`，为 0 则结束 | 3 种退出条件（完美/稳定/超时） |
| 完成后清理 | `DELETE FROM T_BLOCK WHERE REMOTE_ID!='%s'` + `exit(0)` | T_BLOCK 永久保留，仅 `end_session` 显式清理 |
